...
export enum GatePassStatus {
  ...
  PENDING_ACADEMIC_DIRECTOR_FROM_STAFF = 'PENDING_ACADEMIC_DIRECTOR_FROM_STAFF',
  PENDING_ACADEMIC_DIRECTOR_FROM_HOD = 'PENDING_ACADEMIC_DIRECTOR_FROM_HOD',
  APPROVED = 'APPROVED',
  REJECTED = 'REJECTED',
...
// Interface for creating staff gate pass
export interface CreateStaffGatePassDto extends CreateGatePassDto {
  requester_type?: RequesterType;
}
...
export const gatePassApi = {
  ...
  createForStaff: async (token: string, gatePassData: CreateStaffGatePassDto): Promise<GatePass> => {
    try {
      console.log('Creating staff gate pass with data:', JSON.stringify(gatePassData));
      const authAxios = createAuthorizedAxios(token);
      
      // Set the requester_type to STAFF if not already set
      const payload = {
        ...gatePassData,
        requester_type: RequesterType.STAFF
      };
      
      // Fix the URL path to ensure it doesn't use a leading slash
      const response = await authAxios.post('staff', payload);
      return response.data;
    } catch (error) {
      console.error('Error creating staff gate pass:', error);
